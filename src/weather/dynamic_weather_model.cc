...
dynamic_weather_model::get_forward_model_function() {
  std::function<Eigen::VectorXd(const Eigen::VectorXd&)> forward_model =
      std::bind(&dynamic_weather_model::forward_model, this,
                std::placeholders::_1, this->parameter_locations,
...
/**
 * Evaluate the generative model for the weather problem. it is strongly based
 * on the FAIR climate model https://homepages.see.leeds.ac.uk/~mencsm/fair.htm
 *
 * This is a simple climate model, based on energy balance. The model predicts
 * changes in atmpospheric temperature based on the balance of energy recieved
 * and emitted from the earth, as modified by greenhouse gasses, and other
 * effects.
 *
 * The model has 4 layers, 1 atmospheric layer and 3 (progressively deeper) sea
 * layers.
 *
 *
 */
Eigen::MatrixXd dynamic_weather_model::eval_generative(
  ...
  // Initialize variables for important quantities
  ...
  // Initialize emissions with corresponding parameters from the parameters
  // array
  emissions_array(0, Eigen::seq(250, Eigen::last)) =
      parameters(Eigen::seq(11, Eigen::last)).transpose();

  // Calculate the energy balance matrix, our simple system model
...
//       emissions_array(species_list.co2_ffi_indices, Eigen::all);
  ...
  // Summed forcings (at each timestep)
  ...
  // How one unit of forcing manifests changes in temperature
  ...
  // Air/Sea temperature state array
  ...
  // Calculate offset for forcing relationships
  ...
  for (int i = 0; i < timeseries_length - 1; i++) {
    ...
    // Update gas concentrations based on emissions
    ...
    // Increment cumulative emissions
    cumulative_emissions_array(Eigen::all, i + 1) =
        cumulative_emissions_array(Eigen::all, i) +
        emissions_array(Eigen::all, i) * 1;

    // Calculate ghg forcings, and apply pre-calculated offset
    ...
    // Forcing from aerosol-cloud interactions
    ...
    // Sum forcings and apply efficacy
    ...
    // Step temperature from forcings
  ...
  // Pack outputs into a vector and return
